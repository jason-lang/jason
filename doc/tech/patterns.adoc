...
NOTE: The annotation `atomic` could be considered for the first plan. However it will constraint too much the agent execution: no other intention will run until `+!g` finished.

== "Queued" Plan

As for the singleton plan, we want that only one intention is executing the plan. However, when an intention has no access to the plan, it waits in for the running intention to complete and then tries again to execute.
----
+!g : not .intend(g)
   <- dosomething;
...
We can notice no interleaving among the execution of the intentions.
