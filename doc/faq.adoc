...
and if it fails, it still tries to generate an internal event  `pass:c[+?p]`.
...
`pass:c[+?p]` as triggering event, then that plan could be executed to satisfy
the test goal. Events `-!p` and `-?p` are only generated if an
executing plan for `pass:c[+!g]` and `+?g` (respectively) fail. Here’s what
...
executed to handle an internal event with a goal addition `pass:c[+!g]`, then
...
intention stack. However, if a plan for `pass:c[+!g2]` failed and there were
