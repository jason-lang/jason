...
- You can now execute the Jason IDE by going to the folder `jedit` and double-clicking on `jedit.jar`.footnote:[Eclipse can also be used as the IDE, see http://jason.sourceforge.net/mini-tutorial/eclipse-plugin[here].] For Linux and MacOS, the script `jason-ide` can be also used (see more instructions http://jason.sourceforge.net/mini-tutorial/getting-started/shell-based.html[here]).
