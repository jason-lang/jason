...
----
+
The `jadedf.register("book-selling","JADE-book-trading")`
  code calls the internal action named `register` in a package
  called `jadedf`.

- To create this internal action, click on
  the image:./figures/createIA.gif[] ican and fill in the form
  as follows:
+
image:./figures/screen-create-ia.png[]
+
The source code of this internal action is as follows (the
  `execute` method has the code for the internal action and
  `args` contains the arguments given in the AgentSpeak code):
+
[source,java]
.jadedf/register.java
----
include::book_trading/jadedf/register.java[]
----
+
...
----
<1> Where XXXX is the release of Jade in the `libs` folder.
