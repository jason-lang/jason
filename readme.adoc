...
NOTE: Jason 2.0 changed the way to run the Jason IDE (based on jEdit). Instead of running an application (Jason.exe or Jason.app), the IDE is run by double clicking on `jedit/jedit.jar` file.
