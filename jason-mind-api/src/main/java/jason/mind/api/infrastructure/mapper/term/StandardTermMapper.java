...
public final class StandardTermMapper implements TermMapper {

    public StandardTermMapper() {}
    ...
    public NamedValueTermWrapper extractVar(VarTerm varTerm, Unifier unifier) {
        ...
        String name = varTerm.isUnnamedVar() ? UNNAMED_TERM_NAME : varTerm.getFunctor();

        return new NamedValueTermWrapper(name,
                values == null || values.isEmpty() ? null : values.size() == 1 ? values.get(0) : values,
                TermType.var,
                src);
    }
    ...
    public void extractAll(List<TermWrapper> currentTerms, Term currentTerm, Unifier unifier) {
        ...
        if (currentTerm.isNumeric() && !currentTerm.isArithExpr()) {
            ...
            currentTerms.add(new ValueTermWrapper(TermType.string, ((StringTermImpl) currentTerm).getString(),
